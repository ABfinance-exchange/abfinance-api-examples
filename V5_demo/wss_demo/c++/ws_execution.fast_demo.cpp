...
int main()
{
    ...
    Exchange abfinance{ "abfinance", "stream-testnet.abfinance.com" };

    try {
        abfinance.init_webSocket("stream-testnet.abfinance.com", "443", "/v5/private");

        abfinance.authenticate();

        if (abfinance.is_socket_open()) {
            std::string subscription_message = R"({"op": "subscribe", "args": ["execution.fast"]})";
            abfinance.write_Socket(subscription_message);
        }
        while (true) {
            abfinance.read_Socket();
            std::cout << abfinance.get_socket_data();

            abfinance.buffer_clear();
        }
        abfinance.webSocket_close();
    }
