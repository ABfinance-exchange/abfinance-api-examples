...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let api_secret = "your_secret_key_here";
...
fn get_open_order(api_key: &str, api_secret: &str, timestamp: &String, recv_window: &str) -> Result<(), Box<dyn Error>> {
    ...
    let response = client.get(&format!("https://api-testnet.abfinance.com/v5/order/realtime?{}", query_str))
...
fn place_order(api_key: &str, api_secret: &str, timestamp: &String, recv_window: &str) -> Result<(), Box<dyn Error>> {
    ...
    let response = client.post("https://api-testnet.abfinance.com/v5/order/create")
        .json(&params)
        .header("X-BAPI-API-KEY", api_key)
        .header("X-BAPI-SIGN", &signature)
        .header("X-BAPI-SIGN-TYPE", "2")
