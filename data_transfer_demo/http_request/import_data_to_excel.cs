...
namespace ABFinanceApiExample
{
    class Program
    {
        private const string ApiKey = "xxxxxxxxx";
        private const string SecretKey = "xxxxxxxxxxxxxxxxx";
        private const string Url = "https://api-testnet.abfinance.com";

        static async Task Main(string[] args)
        {
            ExcelPackage.LicenseContext = LicenseContext.NonCommercial;
            var endPoint = "/v5/market/tickers";
            var payload = "category=spot";
        ...
        private static async Task HttpRequest(string endPoint, HttpMethod method, string payload)
        {
            ...
            ExportDataToExcel(responseObject);
        }

        public static void ExportDataToExcel(ApiResponse? responseObject)
        {
            if (responseObject?.Result?.List != null && responseObject.Result.List.Count > 0)
            {
                ...
                var headers = typeof(MarketData).GetProperties();
                int column = 1;
                ...
                // Populate data
                int row = 2;
                foreach (var item in responseObject.Result.List)
                {
                    column = 1;
                    foreach (var property in headers)
                    {
                ...
                // Save the Excel file
                var fileInfo = new FileInfo("ABFinanceMarketDataTicker.xlsx");
